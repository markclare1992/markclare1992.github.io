...
Looking at the entry with the max time_to_rebound, here, we can see that a penalty rebound does occur, and the time logging of when the penalty is taken may be the issue.
...
Once again looking at the rebound with the most extreme x position, here, we can be confident that this fits the description of a penalty rebound.
