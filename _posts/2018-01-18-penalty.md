...
### Origin
With a large dataset it is fairly easy to calculate a quick estimate for how likely it is that a penalty will result in a goal.  Looking at the outcome of penalties awarded, excluding penalty shootouts (they're converted at a different rate), we can see where the widely used figure of 0.76 comes from.
...
Once again looking at a rebound with one of the most extreme x positions (x=71, y=58), we can be confident that this fits the description of a penalty rebound.
